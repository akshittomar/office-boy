          ...
          <div className=" card-footer d-flex justify-content-between container" >
            ...
            <div className='container'>
              {hh === 0 && mm === 0 && ss === 0
                ...
                : <h6>
                  <div className="dropdown container"><button  className="btn btn-secondary btn-sm dropdown-toggle" type="button" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false">Time Left:{hh < 10 ? `0${hh}` : hh}:{mm < 10 ? `0${mm}` : mm}:{ss < 10 ? `0${ss+"   "}    ` : ss+"   "}    <i className="fa-solid fa-trash" onClick={() => {
                  var s1 = note.title + "sec";
                  var s2 = note.title + "min";
                  var s3 = note.title + "hrs";
                  localStorage.removeItem(s1);
